...
// QueryPresets chains the current query on the "presets" edge.
func (rq *ReviewQuery) QueryPresets() *PresetQuery {
	query := (&PresetClient{config: rq.config}).Query()
...
func (rq *ReviewQuery) Clone() *ReviewQuery {
	...
	return &ReviewQuery{
		...
		withPresets: rq.withPresets.Clone(),
...
// WithPresets tells the query-builder to eager-load the nodes that are connected to
// the "presets" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *ReviewQuery) WithPresets(opts ...func(*PresetQuery)) *ReviewQuery {
	query := (&PresetClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withPresets = query
	return rq
}
...
func (rq *ReviewQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Review, error) {
	...
	if query := rq.withPresets; query != nil {
		if err := rq.loadPresets(ctx, query, nodes,
			func(n *Review) { n.Edges.Presets = []*Preset{} },
			func(n *Review, e *Preset) { n.Edges.Presets = append(n.Edges.Presets, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *ReviewQuery) loadPresets(ctx context.Context, query *PresetQuery, nodes []*Review, init func(*Review), assign func(*Review, *Preset)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(review.PresetsTable)
		s.Join(joinT).On(s.C(preset.FieldID), joinT.C(review.PresetsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(review.PresetsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(review.PresetsPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*Preset](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "presets" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
